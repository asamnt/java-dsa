...
public class LeadersInArray {
    ...
    public static void printLeadersInArray(int[] arr) {
        ...
        for (int i = 0; i < arr.length; i++) {
            ...
            for (int j = i+1; j < arr.length; j++) {//we compare each element with all the elements in the array
                if (arr[i] <= arr[j]) {
                    //if the next element is greater than current element being compared then the current element is not a leader
    ...
    public static void printLeadersInArrayEfficient(int[] arr) {
        ...
        for(int i=arr.length-2; i >0; i--){
            if (currLeader < arr[i]) { // this means curr leader(the item to the right) is less than current i element(item to the left)
