...
public class LeftRotateArrayByOne {
    ...
    private static int[] leftRotateByDEfficient(int[] arr, int d) {
        ...
        for (int i = 0; i < temp.length; i++) {// temp array has d elements so we loop d times
            ...
            // why n-d+1 =>
            // we have already moved the remaining n-d elements, so they are in their
            // correct index from 0 to n-d-1
            // so we start filling the array from the next index of that i.e from (n-d+i)
            // here i starts with 0
        }
        return arr;
    }
